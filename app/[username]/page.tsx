...
export default function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const inputClass = "w-full rounded-xl px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-sky-500 transition"
    ...
    <div className="max-w-2xl mx-auto px-4 py-10 pb-24">
      ...
      {showMutuals && !isOwn && mutualData && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center" onClick={() => setShowMutuals(false)}>
          <div className="w-full max-w-lg rounded-t-2xl pb-8" style={{ background: "#0D1640", border: "1px solid #ffffff15" }} onClick={e => e.stopPropagation()}>
              ...
              {mutualData.users.map(u => (
                <button key={u.id} onClick={() => { setShowMutuals(false); router.push(`/@${u.username}`) }}
                  ...
                  {u.image
                    ...
                    : <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 text-sm" style={{ background: "linear-gradient(135deg, #00D4FF 0%, #6B5EFF 50%, #FF5EE8 100%)" }}>{(u.name ?? u.username ?? "?")[0].toUpperCase()}</div>
                  }
                  <div>
      ...
      <div className="flex items-start gap-6 mb-8">
        {profileUser.image ? (
          ...
          <div className="w-20 h-20 rounded-full flex items-center justify-center text-white text-2xl font-bold flex-shrink-0" style={{ background: "linear-gradient(135deg, #00D4FF 0%, #6B5EFF 50%, #FF5EE8 100%)" }}>
            {initials}
          </div>
        )}
              ...
              {posts.map((post) => (
                <button key={post.id} onClick={() => setViewPost(post as PostItem)}
                  ...
                  <div className="absolute top-1.5 left-1.5 flex gap-1">
                    {post.isAiGenerated && (
                      <span className="text-xs font-medium bg-sky-600/80 text-white px-1.5 py-0.5 rounded-md">AI</span>
                    )}
        ...
        <>
          ...
          {shopLoading ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              {shopItems.map((item) => (
                <div key={item.id} className="rounded-2xl overflow-hidden" style={{ background: "#0D1640", border: "1px solid #ffffff15" }}>
      ...
      {tab === "Commissions" && (
        <>
          {!commissionProfile ? (
            ...
            <>
              {/* Info card */}
              <div className="rounded-2xl p-5 mb-6" style={{ background: "#0D1640", border: "1px solid #ffffff15" }}>
                {/* Trust score */}
      ...
      {showUpload && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="rounded-2xl w-full max-w-md flex flex-col gap-4 p-6 max-h-[90vh] overflow-y-auto" style={{ background: "#0D1640", border: "1px solid #ffffff15" }}>
            ...
            <textarea value={uploadDesc} onChange={(e) => setUploadDesc(e.target.value)}
              ...
              className="w-full rounded-xl px-3 py-2 text-sm text-white placeholder-white/30 resize-none focus:outline-none focus:ring-2 focus:ring-sky-500"
            ...
            {[
              ...
              <div key={label} className="flex items-center justify-between py-1">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${value ? "bg-sky-600" : "bg-white/20"}`}
