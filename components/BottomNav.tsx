...
// Gradient matching the Gallery "G" logo — applied to all icons (via SVG gradient) and labels (via background-clip)
...
function SearchModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
      <div className="w-full max-w-lg rounded-t-2xl md:rounded-2xl p-5 pb-8" style={{ background: "#0D1640", border: "1px solid #ffffff15" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-4">
          <input
            ...
            className="flex-1 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none focus:ring-2 focus:ring-sky-500"
            ...
            {users && users.length > 0 && users.map((user) => (
              ...
              >
                {user.image ? (
                  ...
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0" style={{ background: "linear-gradient(135deg, #00D4FF 0%, #6B5EFF 50%, #FF5EE8 100%)" }}>
            ...
            notifications.map((n) => {
              ...
              return (
                <button
                  ...
                  style={{ background: !n.read ? "rgba(107,94,255,0.08)" : "transparent" }}
                  onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.05)")}
                  onMouseLeave={e => (e.currentTarget.style.background = !n.read ? "rgba(107,94,255,0.08)" : "transparent")}
                >
...
export default function BottomNav() {
  ...
  const navItems = [
    {
      ...
      active: pathname === "/",
      icon: (
        <svg width="24" height="24" fill="none" stroke="url(#nav-grad)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      icon: (
        <svg width="24" height="24" fill="none" stroke="url(#nav-grad)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
  ...
  const notifBadge = unread && unread.count > 0 ? unread.count : null
  const dmBadge = dmUnread && dmUnread.count > 0 ? dmUnread.count : null

  return (
    <>
      {/* Hidden SVG gradient definition — referenced as url(#nav-grad) by all icons */}
      <svg width="0" height="0" style={{ position: "absolute", overflow: "hidden" }} aria-hidden="true">
        <defs>
          <linearGradient id="nav-grad" x1="0" y1="0" x2="24" y2="24" gradientUnits="userSpaceOnUse">
      ...
      {/* ── MOBILE bottom nav ─────────────────────────────────────────────── */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around px-2 h-16" style={{ background: "#070C1C", borderTop: "1px solid #ffffff10" }}>
        {navItems.map((item) => {
          const inner = (
            <div className="flex flex-col items-center gap-0.5 px-2 py-2">
              ...
              <span className="text-[10px] font-medium" style={gradStyle}>{item.label}</span>
            </div>
          )
          if (item.onClick) return <button key={item.label} onClick={item.onClick}>{inner}</button>
        ...
        <button onClick={() => setNotifOpen(v => !v)}>
          <div className="flex flex-col items-center gap-0.5 px-2 py-2">
            ...
            <span className="text-[10px] font-medium" style={gradStyle}>Activity</span>
          </div>
        </button>
      </nav>
      ...
      <nav
        ...
        style={{ width: "64px", background: "#070C1C", borderRight: "1px solid #ffffff10" }}
      ...
      >
        ...
        <Link href="/" className="mb-8 flex items-center gap-3 flex-shrink-0 overflow-hidden">
          ...
          <span
            className="text-lg font-bold tracking-tight whitespace-nowrap overflow-hidden transition-all duration-200 opacity-0 group-hover:opacity-100 w-0 group-hover:w-auto"
            style={gradStyle}
          >
            Gallery
          </span>
        ...
        <div className="flex flex-col gap-0.5 flex-1">
          {navItems.map((item) => {
            const label = (
              <span
                className="text-[15px] font-medium whitespace-nowrap overflow-hidden transition-all duration-200 opacity-0 group-hover:opacity-100 w-0 group-hover:w-auto"
                style={gradStyle}
              >
                {item.label}
              </span>
            ...
            const inner = (
              <div
                ...
                style={item.active ? { background: "linear-gradient(135deg, #00D4FF20, #6B5EFF20, #00B4EE20)" } : {}}
              >
